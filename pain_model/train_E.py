...
bhvs, mouse_ids = load_all('normal')


def action_from_bhv(bhv,time):
    bhv_ = bhv[1:]-bhv[0:-1]
    begins = np.where(bhv_ == 1)[0]/20
    ends = np.where(bhv_ == 255)[0]/20
    if time in begins:
        idx = np.where(begins==time)[0][0]
        return torch.tensor([[0]], device=device) , round(ends[idx]-begins[idx])
    else:
        return torch.tensor([[1]], device=device) , 0
...
def step(pain, energy, x, action, time, sig, NPY, pain_profile):
    ...
    energy = energy - energy*conv_param[1] + (1-action)   #/(1+(NPY>0))
    pain = nonlinearity(x)
...
def step_for_duration(pain, energy, x, duration, time, sig, NPY, pain_profile):
    ...
    for t in range(duration):
        ...
        x = x + (-x+pain)/5 + P_input - 0.007 #- NPY
        energy = energy - energy*conv_param[1] + 1     #/(1+(NPY>0))   
        pain = nonlinearity(x)
...
def run_episode(agent, eps, pain_profile):
    ...
    """
    NPY = 0.0011*(np.random.rand() > 0.75)
    ...
    while i < time_steps:
        ...
        if action.item() == 0:
            ...
            reward = hrll_reward(state[0], (1+1.*(NPY>0))*conv_param[0]*state[1], next_state1, (1+1.*(NPY>0))*conv_param[0]*next_state2)
...
def run_sim(agent, eps, NPY, pain_profile):
    ...
    while i < time_steps:
        ...
        reward = imm_reward(next_state1, (1+1.*(NPY>0))*conv_param[0]*next_state2)
        total_reward += reward
        x_list.append(x)
    ...
    for i in range(nb):
        for j in range(nb):
            action = agent.get_action(FloatTensor(
                [np.array([0.0025*i, 0.0025*j])]), 0.00)
...
# v10 is with doubling of E time constant when NPY is on
# v11 is with doubling amplitude of E effect when NPY is on 

name = 'RL_lick_expbouts_v11_7'
print(name)
...
# visualize actions
NPY = 0
st, act, rew, x = run_sim(agent, 0, NPY, np.array([3.5, 2000, 660]))
...
ax.plot(np.arange(0,60,60/3600),(1+1.*(NPY>0))*conv_param[0]*np.array(st[0:3600])[:, 1], linewidth=2.5)
#ax.plot(x)
...
ax.invert_yaxis()
ax.tick_params(axis='both',which='major',labelsize=20)
plt.plot((1+1.*(NPY>0))*conv_param[0]*400.0*np.array(st)[:, 1],
