...
def step(pain, energy, x, action, time, pain_profile):
...
def run_sim(agent, eps, hunger, pain_profile, T):
    ...
    while i < time_steps:
        ...
        if action.item() == 0:    
            ...
            for t in range(bout_dur):
                ...
                next_state1, next_state2, x = step(state[0], state[1], x, action.item(), i, pain_profile)
...
def plot_bhv_hunger(bhv,bhv_hunger,its,args):
    ...
    ax.plot(labels, np.mean(licking, axis=0), linewidth='5',color='gray', label='ad libitum model')
    ax.fill_between(labels,
                     np.mean(licking, axis=0)-np.std(licking, axis=0)/np.sqrt(8),
                     np.mean(licking, axis=0)+np.std(licking, axis=0)/np.sqrt(8),
                     alpha = 0.4, linewidth=0, color='gray')
    ax.plot(labels, np.mean(licking_hunger, axis=0), linewidth='5',color='darkblue',label=args+' model')
    ax.fill_between(labels,
...
def plot_avg_PE_comparison(states_list1,states_list2,dim,args):
    ...
    if dim ==0:
        ...
        Es*=conv_param[0]
    fig, ax = plt.subplots(figsize=(10,8))
    ax.plot(np.arange(0,60,60/T),np.mean(Ps, axis=0), linewidth='4',color='gray', label='ad libitum model')
    ax.fill_between(np.arange(0,60,60/T), np.mean(Ps, axis=0)-np.std(Ps, axis=0)/np.sqrt(N),
                     np.mean(Ps, axis=0)+np.std(Ps, axis=0)/np.sqrt(N),
                     alpha = 0.4, linewidth=0, color='gray')
    
    ax.plot(np.arange(0,60,60/T),np.mean(Es, axis=0), linewidth='4',color='darkblue', label=args+' model')
    ax.fill_between(np.arange(0,60,60/T), np.mean(Es, axis=0)-np.std(Es, axis=0)/np.sqrt(N),
                     np.mean(Es, axis=0)+np.std(Es, axis=0)/np.sqrt(N),
...
def plot_one_PE_sum_comparison(states_list1,states_list2,args):
    ...
    ax.legend()
    ax.legend(frameon=False)
    plt.savefig('figs_for_paper/comparison_one_sum_model_'+args+'.pdf', format='pdf',bbox_inches='tight')
...
def plot_avg_PE_sum_comparison(states_list1,states_list2,args):
    ...
    ax.legend()
    ax.legend(frameon=False)
    plt.savefig('figs_for_paper/comparison_sum_model_'+args+'.pdf', format='pdf',bbox_inches='tight')

    plt.show()      

def plot_bar(bhv1,bhv2,arg):
    if arg == 1:
    ...
    plt.savefig('figs_for_paper/barplot_phase'+str(arg)+'.pdf',format='pdf',bbox_inches='tight')
    plt.show()
...
bhv_tmt = np.zeros((its,T))
st = []
st_hunger = []
st_tmt = []

#choose policy manipulation: (figure 4 and extended data figure 10)
#Manipulation 1: 'H1', Manipulation 2: 'H2'
args = 'H2'
for it in range(its):
    ...
    pain_profile = np.array([3.5, 1800+np.random.rand()* 400, 560+np.random.rand()*200])
    
    NPY = 0.
    state_list, action_list, _ = run_sim(agent, 0.0, NPY, pain_profile, T)
