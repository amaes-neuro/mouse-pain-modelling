...
def step(pain, energy, x, action, time, pain_profile, arg):
    ...
    if arg == 'P1':
        x = x + (-x+pain)/5 + P_input - 0.007*(1-action) - NPY
    elif arg == 'P2':
...
def run_sim(agent, eps, pain_profile, T, arg):
    ...
    while i < time_steps:
        ...
        if action.item() == 0:    
            ...
            for t in range(bout_dur):
                ...
                next_state1, next_state2, x = step(state[0], state[1], x, action.item(), i, pain_profile, arg) 
                next_state = np.array([next_state1,next_state2])
                state = next_state
                states_list.append(state[0:2])
            next_state1, next_state2, x = step(next_state1, next_state2, x, 1, i, pain_profile, arg)
            i = i + 1
...
def plot_nonlinearity():
    values = np.zeros((121,1))
    for i in range(-20,100):
        values[i+21] = nonlinearity(i/200)
    fig, ax = plt.subplots(figsize=(10,3.5))
    ax.plot(np.linspace(-0.1,0.5,121),values,linewidth='5',color='black')
    ax.spines.right.set_visible(False)
    ax.spines.top.set_visible(False)
    ax.set_xlabel('x',fontsize=28)
    ax.set_ylabel('f(x)',fontsize=28)
    ax.tick_params(axis='x', labelsize=25)
    ax.tick_params(axis='y', labelsize=25)
    ax.set_xlim([-0.1,0.5])
    ax.set_ylim([0,0.6])
    plt.savefig('figures/nonlinearity.pdf', format='pdf',bbox_inches='tight')
    plt.show()
    
    
def plot_pain_input(pain_profile):
    P_input = 4/(np.sqrt(2*np.pi*sig**2))*np.exp(-0.5*np.linspace(0,3599,3600)**2/sig**2) + pain_profile[0]*1/(np.sqrt(2*np.pi*pain_profile[2]**2))*np.exp(-0.5*(np.linspace(0,3599,3600)-pain_profile[1])**2/pain_profile[2]**2)    

    fig, ax = plt.subplots(figsize=(10,3.5))
    ax.plot(np.arange(0,60,60/T),P_input,linewidth='5',color='black')
    ax.spines.right.set_visible(False)
    ax.spines.top.set_visible(False)
    ax.set_xlabel('Time (min)',fontsize=28)
    ax.set_ylabel('Pain input',fontsize=28)
    ax.tick_params(axis='x', labelsize=25)
    ax.tick_params(axis='y', labelsize=25)
    ax.set_xlim([0,60])
    ax.set_ylim([0,0.02])
    plt.savefig('figures/pain_input.pdf', format='pdf',bbox_inches='tight')
    plt.show()
...
def plot_one_PE_sum_comparison(states_list1,states_list2,args):

    PEs = np.array(states_list1)[:,0]+conv_param[0]*np.array(states_list1)[:,1]
    PEs_hunger = np.array(states_list2)[:,0]+conv_param[0]*np.array(states_list2)[:,1]
    
    fig, ax = plt.subplots(figsize=(10,8))
    ax.plot(np.arange(0,60,60/T),PEs, linewidth='4', color='gray', label='ad libitum model')
    
    ax.plot(np.arange(0,60,60/T),PEs_hunger, linewidth='4', color='darkblue', label='hunger model')
    ...
    ax.legend()
    ax.legend(frameon=False)
    plt.savefig('figures/comparison_one_sum_model_'+args+'.pdf', format='pdf',bbox_inches='tight')
...
def plot_avg_PE_sum_comparison(states_list1,states_list2,args):
    ...
    for i in range(N):
       ...
       PEs_hunger[i,:] = np.array(states_list2[i])[:,0]+conv_param[0]*np.array(states_list2[i])[:,1]
    
    fig, ax = plt.subplots(figsize=(10,8))
    ax.plot(np.arange(0,60,60/T),np.mean(PEs, axis=0), linewidth='4',color='gray', label='Control')
    ...
    ax.plot(np.arange(0,60,60/T),np.mean(PEs_hunger, axis=0), linewidth='4',color='darkblue', label='Competing needs')
    ...
    ax.legend()
    ax.legend(frameon=False)
    plt.savefig('figures/comparison_sum_model_'+args+'.pdf', format='pdf',bbox_inches='tight')

    plt.show()  
    
    
def plot_bar(bhv1,bhv2,arg, arg2):
    if arg == 1:
        y = [np.sum(-1*(bhv1[:,0:300]-1),axis=1),np.sum(-1*(bhv2[:,0:300]-1),axis=1)]
    elif arg == 2:
        y = [np.sum(-1*(bhv1[:,901:3600]-1),axis=1),np.sum(-1*(bhv2[:,901:3600]-1),axis=1)]
    w = 0.6
    x =[1,2]
    ...
    plt.savefig('figures/barplot_phase'+str(arg)+'_'+arg2+'.pdf',format='pdf',bbox_inches='tight')
    plt.show()
...
#choose manipulation: (figure 4 and extended data figure 10)
#Ad Libitum only: 'AL', Manipulation 1 for pain variable: 'P1', Manipulation 2 for pain variable: 'P2'
args = 'P1'
...
plot_bar(bhv,bhv_hunger,1, args)
